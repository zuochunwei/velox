...
ColumnReader::ColumnReader(
  ...
  auto stream = stripe.getStream(id, streamLabels.label(), false);
  if (stream) {
...
class ByteRleColumnReader : public ColumnReader {
 ...
 public:
  ByteRleColumnReader(
    ...
    rle =
        creator(stripe.getStream(id, streamLabels.label(), true), encodingKey);
  }
...
StringDictionaryColumnReader::StringDictionaryColumnReader(
  ...
  if (format == DwrfFormat::kDwrf) {
    ...
    dictionaryId = encodingKey.forKind(proto::orc::Stream_Kind_DICTIONARY_DATA);
  }
